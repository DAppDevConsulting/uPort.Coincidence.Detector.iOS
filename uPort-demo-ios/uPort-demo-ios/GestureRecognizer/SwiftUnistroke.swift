...
public class SwiftUnistroke {
    ...
    public func recognizeIn(templates: [SwiftUnistrokeTemplate]!, useProtractor: Bool = false, minThreshold: Double = 0.70) throws -> (template: SwiftUnistrokeTemplate?, distance: Double?) {
        ...
        self.points = StrokePoint.resample(points: points, totalPoints: StrokeConsts.numPoints)
        let radians = StrokePoint.indicativeAngle(points: self.points)
        self.points = StrokePoint.rotate(points: self.points, byRadians: -radians)
        self.points = StrokePoint.scale(points: self.points, toSize: StrokeConsts.squareSize)
...
public struct StrokePoint {
    ...
    public static func zeroPoint() -> StrokePoint {
        return StrokePoint(point: CGPoint.zero) //StrokePoint(point: CGPointZero)
    }
...
extension StrokePoint {
    ...
    public static func pathLength(points: [StrokePoint]) -> Double {
        ...
        for idx in 1 ..< points.count {
            totalDistance += points[idx-1].distanceTo(point: points[idx])
        }
    ...
    public static func pathDistance(path1: [StrokePoint], path2: [StrokePoint]) -> Double {
        ...
        for idx in 0 ..< path1.count {
            if idx > path1.count - 1 || idx > path2.count - 1 {
                break
            }
